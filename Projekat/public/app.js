...
ul.addEventListener('click',event =>{
    ...
    if(event.target.classList.includes('obrisi')){
    }
})

let formColor = document.querySelector('#formOdabirBoje');
let izborBoje = document.getElementById('inputBoja');
let glavni = document.getElementById('glavni');

formColor.addEventListener('click',event =>{

    if(event.target.id === "btnColor"){
        event.preventDefault();
        setTimeout(function(){
            let boja = izborBoje.value;
            glavni.style.backgroundColor = boja;
            objekat1.updateColor(boja);
        },500)
        }
})
...
let formDateTime = document.querySelector('#dateTime');
let inDate = document.getElementById('inputDate');
let inTime = document.getElementById('inputTime');
let submitDateTime = document.getElementById('updateDateTime');

submitDateTime.addEventListener('click',event =>{
    event.preventDefault();
    let checkDate = inDate.value;
    let checkTime = inTime.value;
})
